...
fichier.write("-----------------")
fichier.write("\n")
vehicle = connection()# Connection au drone
debut=time.time()
chaine=0
fl=0
...
def lecture(serialArduino,fl):
    ...
    while event==0:
        print("event", event)
        chrono1=time.time()
        chaine,PM1,PM2_5,PM10=ReadOPCData(serialArduino,fl) # lecture des donnees sur le bus serie
        print("ok2")
...
while True:   
	...
	while True:   #Parcours des points GPS de l'ellipse 
		...
		tour=Traj(VitesseCercle,Seuil_PM1_entree,Seuil_PM_2_5_entree,Seuil_PM1_sortie,
