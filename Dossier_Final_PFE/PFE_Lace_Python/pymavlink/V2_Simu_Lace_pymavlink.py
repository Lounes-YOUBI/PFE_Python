...
# Définir l'origine GPS pour la conversion
ORIGIN_LAT = 48.581918  # Exemple : Paris
ORIGIN_LON = 7.763784

# Modèle SDF d'une sphère
model_template = """<sdf version='1.6'>
<model name='{name}'>
  <static>true</static>
  <link name='link'>
    <visual name='visual'>
...
def gps_to_gazebo(lat, lon, alt):
    """ Convertit GPS (lat, lon, alt) en coordonnées locales Gazebo (x, y, z) """
    utm_pos = utm.from_latlon(lat, lon)
    utm_origin = utm.from_latlon(ORIGIN_LAT, ORIGIN_LON)

    x = utm_pos[0] - utm_origin[0]
    y = utm_pos[1] - utm_origin[1]
    z = alt  # L'altitude reste inchangée

    return x, y, z


def spawn_point(lat, lon, alt):
    """ Crée un point en Gazebo basé sur une position GPS avec un index automatique """
    global index_counter  # Permet de modifier la variable globale
    rospy.wait_for_service("/gazebo/spawn_sdf_model")
    spawn_model = rospy.ServiceProxy("/gazebo/spawn_sdf_model", SpawnModel)

    x, y, z = gps_to_gazebo(lat, lon, alt)
    model_sdf = model_template.format(index_counter, x, y, z)

    pose = Pose()
    spawn_model(f"point_{index_counter}", model_sdf, "", pose, "world")

    index_counter += 1  # Incrémenter l'index pour le prochain point
...
async def simple_goto(lat, long, alt):
    ...
    # On affiche le point cible
    spawn_point(lat, lon, alt)
