...
def wait_for_arm(timeout=15):
	print("Attente de l'armement du drone (mode ArduPlane)...")
	start = time.time()
	while time.time() - start < timeout:
		hb = master.recv_match(type='HEARTBEAT', blocking=True, timeout=2)
		if hb:
			armed = bool(hb.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED)
			print(f"  -> Drone {'ARMÉ' if armed else 'non armé'}")
			if armed:
				return True
		time.sleep(1)
	print("⛔️ Timeout : le drone ne s'est pas armé.")
	return False


def arm_and_takeoff_quadplane(aTargetAltitude):
	"""
	Arms a quadplane VTOL and performs vertical takeoff to aTargetAltitude.
	"""

	set_param('RTL_AUTOLAND', 1)
	...
	print("Setting mode to QLOITER")
	set_mode("QLOITER")  # QLOITER est l'équivalent "GUIDED" pour un quadplane

	print("Arming motors")
	master.arducopter_arm()
	print("Commande d'armement envoyée ...")
	if not wait_for_arm():
		raise RuntimeError("Impossible d'armer le drone (timeout).")
	print("Drone armé")

	print("Taking off vertically")
	# DO_VTOL_TAKEOFF fonctionne pour les quadplanes
	master.mav.command_long_send(
		master.target_system,
		master.target_component,
		84, # 84 = MAV_CMD_DO_VTOL_TAKEOFF
		0,
		0, 0, 0, 0, 0, 0, aTargetAltitude  # Altitude de consigne
	)
...
# Le drone decolle
while True:
	print("en attente de QLOITER")
	print("mode: %s" % get_mode())
	if (get_mode() == "QLOITER"):  # Attente du passage du mode "AUTO" manuellement sur QGC
		arm_and_takeoff_quadplane(alt)  # Decollage
		break
	time.sleep(0.25)
