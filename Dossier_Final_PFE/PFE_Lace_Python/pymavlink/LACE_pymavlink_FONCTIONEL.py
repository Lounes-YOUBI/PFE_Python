...
# Fonction pour aller à un waypoint
...
def Traj(VitesseCercle, Seuil_entree, Seuil_sortie, SensorValue, centre, angle, RayonCercle, NbPoints, a, b,
	...
	print("Traj - ")
	position = get_position()
	dist, rang = NearestCoord(coords, get_position())
	...
	while True:  # Boucle while
		...
		if ok == 0:
			...
			goto(points[pt].lat_deg, points[pt].lon_deg, points[pt].relative_alt_m)
			start = time.time()
			ok = 1
		nxt = pt - 1
		if nxt == -1:
		...
		# print("D",D)
		if get_distance_metres(position, points[pt]) < fact_dist * D or time.time() - start > D / fact_temps:
			pt = pt + 1
			ok = 0
	i = 0
	Dist_min = 100000
...
centre = Position(lat_deg=37.619, lon_deg = -122.377, relative_alt_m = alt)
...
goto(centre.lat_deg, centre.lon_deg, centre.relative_alt_m)
