...
def generer_coordonnees_ellipse2(rayon, direction_cardinale, nb_points_ellipse, facteur_petit_axe, facteur_grand_axe,
                                angle_rotation):
    """
...
def generer_coordonnees_ellipse(rayon, direction_cardinale, nb_points,
...
def executer_trajectoire_ellipse(seuil_entree_nuage, seuil_sortie_nuage, valeurs_capteur, centre_nuage,
    ...
    # Génération des coordonnées de l'ellipse (liste de tuples)
    coordonnees_ellipse_tuples = generer_coordonnees_ellipse(rayon_ellipse, angle_orientation, nb_points_ellipse, facteur_petit_axe, facteur_grand_axe)

    # Conversion en objets Position pour compatibilité avec trouver_point_plus_proche
    coordonnees_ellipse = [Position(lat_deg=lat, lon_deg=lon, relative_alt_m=altitude_vol) for lat, lon in coordonnees_ellipse_tuples]
    ...
    coordonnees_ordonnees = organiser_points_ellipse(coordonnees_ellipse_tuples, index_proche, -1)

    # Conversion en objets Position pour la trajectoire ordonnée
    points_ellipse = [Position(lat_deg=lat, lon_deg=lon, relative_alt_m=altitude_vol) for lat, lon in coordonnees_ordonnees]
