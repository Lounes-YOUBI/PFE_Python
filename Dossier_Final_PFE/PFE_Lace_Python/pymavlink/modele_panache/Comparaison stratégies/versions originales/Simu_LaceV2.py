...
def Traj(VitesseCercle,Seuil_entree,Seuil_sortie,SensorValue,centre,angle,RayonCercle,NbPoints,a,b,fact_dist,fact_temps,theta,alt):
	...
	while True:
		...
		while i<len(GPSLat):
			...
			while j<len(GPSLon):
				...
				j=j+1
			i=i+1
		#print("capteur",SensorValue[Indice[0]][Indice[1]])	
		if SensorValue[Indice[0]][Indice[1]]> Seuil_entree and get_distance_metres(vehicle.location.global_relative_frame, LocationGlobalRelative(GPSLat[Indice[0]][Indice[1]],GPSLon[Indice[0]][Indice[1]],alt))<15 and ok_j==2:
			ok_j=1
			return True
		elif SensorValue[Indice[0]][Indice[1]]< Seuil_sortie and ok_j==1 and get_distance_metres(vehicle.location.global_relative_frame, LocationGlobalRelative(GPSLat[Indice[0]][Indice[1]],GPSLon[Indice[0]][Indice[1]],alt))<15: 
			ok_j=2		

					
############### Début du programme #########################
#Pos debut simu: 48.628168,7.784950
fichier=open("LogVol.txt","w")
...
arg3= parser.add_argument('--it',type=int, default=5)
...
while True:
	...
	while True:
		...
		vehicle.parameters['WP_YAW_BEHAVIOR'] = 0
		tour=Traj(VitesseCercle,Seuil_entree,Seuil_sortie,SensorValue,centre,angle,RayonCercle,NbPoints,a+a_inc,b+b_inc,fact_dist,fact_temps,theta,alt)
		if tour==True:
			break
		a_inc=a_inc*1.3
		b_inc=b_inc*1.3
