...
arg_nb_points = parser.add_argument('--NbPts', type=int, default=15)
arg_rayon = parser.add_argument('--Rayon', type=float, default=12)
...
# Paramètres du drone
try:
	altitude_vol = args["altitude"]
except:
	altitude_vol = arg_altitude.default

# Paramètres du nuage de particules
seuil_critique = 1500000000  # Seuil critique qui déclenche la procédure de fin de cartographie
...
altitude_coupe = 7  # Altitude à extraire (en mètres)
...
def calculer_diametre_ellipse(coordonnees_ellipse):
	"""
    Calcule le diamètre de l'ellipse.

    Args:
        coordonnees_ellipse: Liste des coordonnées GPS

    Returns:
        float: Diamètre en mètres
    """
	distance_max = 0
	point_reference = Position(lat_deg=coordonnees_ellipse[0][0], lon_deg=coordonnees_ellipse[0][1],
							   relative_alt_m=altitude_vol)

	for i in range(0, len(coordonnees_ellipse)):
		point_courant = Position(lat_deg=coordonnees_ellipse[i][0], lon_deg=coordonnees_ellipse[i][1],
								 relative_alt_m=altitude_vol)
		distance = calculer_distance_metres(point_courant, point_reference)
...
def executer_trajectoire_ellipse(seuil_entree_nuage, seuil_sortie_nuage, valeurs_capteur, centre_nuage,
	...
	#coordonnees_ellipse = generer_coordonnees_ellipse(rayon_ellipse, angle_orientation, nb_points_ellipse,facteur_petit_axe, facteur_grand_axe, angle_rotation)
	coordonnees_ellipse = generer_coordonnees_demi_ellipse(rayon_ellipse, angle_orientation, nb_points_ellipse,facteur_petit_axe, facteur_grand_axe)
	#coordonnees_ellipse = generer_demi_ellipse_avec_segment(rayon_ellipse, angle_orientation, nb_points_ellipse,facteur_petit_axe, facteur_grand_axe)
...
while True:
    ...
    while True:
        ...
        seuil_crit, tour = executer_trajectoire_ellipse(
            seuil_entree,
            seuil_sortie,
            valeurs_capteur,
            centre_nuage,
