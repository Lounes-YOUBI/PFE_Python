...
temps_min_tour_complet = 30  # Temps minimum en secondes avant de considérer qu'un tour complet est possible
...
# Création de la carte (une seule fois, si besoin)
carte_folium = None
...
#################################################################################################################
# PROGRAMME PRINCIPAL MULTI-ALTITUDE (logique strictement identique à l'ancien pour chaque altitude)
#################################################################################################################

for altitude in np.arange(altitude_max, altitude_min - 1e-6, -pas_altitude):
    print(f"\n==== Nouvelle altitude de vol : {altitude} m ====")

    # Réinitialisation des variables de mission pour chaque altitude
    angle_inc = 0
    position_depart_cartographie = None
    temps_depart_cartographie = None
    tour_nuage_complet = False
    dernier_bord_nuage = None

    # Génération du nuage de particules à l'altitude courante
    ...
    # Affichage du nuage sur la carte uniquement à la première altitude (optionnel)
    if carte_folium is None:
    ...
    utils.set_param(master, 'WPNAV_SPEED', 1200)
    ...
    while True:
        ...
        if utils.get_mode(master) == "AUTO":
            ...
            print("Décollage")
            utils.arm_and_takeoff(master, altitude)
            break
        time.sleep(0.5)
    ...
    position_depart_cartographie = entree_nuage
    temps_depart_cartographie = time.time()
    print("Nuage détecté !")
    angle = angle_vent

    # Suivi de frontière à cette altitude
    while True:
        if utils.get_battery_cap(master) < niveau_batterie_min:
            break
        ...
        if tour_nuage_complet == True:
            print("--- Tour du nuage effectué, passage à l'altitude suivante... ---")
            break
        
        time.sleep(0.1)

    print(f"--- Fin de la mission à l'altitude {altitude} m ---")
